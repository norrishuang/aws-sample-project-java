...
/**
 * Flink CDC application for streaming data from MySQL to Elasticsearch.
 * This application captures change data from MySQL using Flink CDC and sinks to Elasticsearch.
 */
public class MySQLCDCToElasticsearch {
    private static final Logger LOG = LoggerFactory.getLogger(MySQLCDCToElasticsearch.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        LOG.info("Starting MySQL CDC to Elasticsearch pipeline");
    ...
    public static class CDCPipeline {
        ...
        public static void createAndDeployJob(
            ...
            try {
                ...
                // Get Elasticsearch configuration parameters (using underscore format for AWS Managed Flink)
                String elasticsearchHost = applicationProperties.get("elasticsearch.host", 
                        "https://<host>:443");
                String elasticsearchIndex = applicationProperties.get("elasticsearch.index", "user_order_list_20cols_new");
                String elasticsearchUsername = applicationProperties.get("elasticsearch.username", "admin");
                String elasticsearchPassword = applicationProperties.get("elasticsearch.password", "ABCDEF");

                LOG.info("MySQL Source Configuration - Host: {}, Port: {}, Database: {}, Table: {}", 
                         mysqlHostname, mysqlPort, mysqlDatabase, mysqlTable);
                LOG.info("Elasticsearch Sink Configuration - Host: {}, Index: {}", 
                         elasticsearchHost, elasticsearchIndex);
                ...
                // Create Elasticsearch Sink Table with the specified schema
                final String elasticsearchSinkDDL = String.format(
                        "CREATE TABLE user_order_list_20cols_new_sink (\n"
                                ...
                                + ");",
                        elasticsearchHost, elasticsearchIndex, elasticsearchUsername, elasticsearchPassword);

                LOG.info("Creating Elasticsearch sink table");
                streamTableEnvironment.executeSql(elasticsearchSinkDDL);

                // Execute data pipeline from MySQL CDC to Elasticsearch
                LOG.info("Starting data pipeline: MySQL CDC -> Elasticsearch");
